...
#include <aes_multiplication_tables.h>
#include <memory.h>

void Rijndael::SubBytes(std::shared_ptr<std::vector<std::vector<uint8_t>>> State)
{
...
void Rijndael::ShiftRows(std::shared_ptr<std::vector<std::vector<uint8_t>>> State)
{
	// Shift 1 row
...
void Rijndael::MixColomns(std::shared_ptr<std::vector<std::vector<uint8_t>>> State)
{
	std::vector<std::vector<uint8_t>> TempState(*State);
...
void Rijndael::AddRoundKey(uint8_t byCurrentRound, std::shared_ptr<std::vector<std::vector<uint8_t>>> State)
{
	for (uint8_t i = 0; i < 4; i++)
	{
		for (uint8_t j = 0; j < 4; j++)
		{
			(*State)[i][j] ^= (*RoundKeys)[byCurrentRound * Nb * Nb + i * Nb + j];
		}
	}
};
...
void Rijndael::KeyExpansion()
{
	...
	while (i < (Nb * (Nr + 1)))
	{
		for (uint8_t j = 0; j < 4; j++)
		{
			byarrWord[j] = (*RoundKeys)[(i - 1) * Nb + j];
		}
		if (i % Nk == 0)
		{
			// RotWord Function
			{
				std::swap(byarrWord[0], byarrWord[3]);
				std::swap(byarrWord[0], byarrWord[1]);
				std::swap(byarrWord[1], byarrWord[2]);
			}
			// SubWord Function
			{
...
void Rijndael::InvShiftRows(std::shared_ptr<std::vector<std::vector<uint8_t>>> State)
{
	// Shift 1 row
	std::swap((*State)[0][1], (*State)[3][1]);
	std::swap((*State)[3][1], (*State)[1][1]);
	std::swap((*State)[3][1], (*State)[2][1]);
	// Shift 2 row
	std::swap((*State)[0][2], (*State)[2][2]);
	std::swap((*State)[1][2], (*State)[3][2]);
	// Shift 3 row
	...
	std::swap((*State)[2][3], (*State)[1][3]);
};

void Rijndael::InvSubBytes(std::shared_ptr<std::vector<std::vector<uint8_t>>> State)
{
	for (uint8_t i = 0; i < 4; i++)
	{
		for (uint8_t j = 0; j < 4; j++)
...
void Rijndael::InvMixColomns(std::shared_ptr<std::vector<std::vector<uint8_t>>> State)
{
	std::vector<std::vector<uint8_t>> TempState(*State);
...
std::vector<uint8_t> Rijndael::Encrypt(const std::vector<uint8_t> &arrbyBlockPlainText, std::shared_ptr<std::vector<uint8_t>> byarrKey)
{
	// Create matrix State with size 4*Nb and Value in Cells is 0
	auto State = std::make_shared<std::vector<std::vector<uint8_t>>>(Nb, std::vector<uint8_t>(Nb, 0));
	...
	std::vector<uint8_t> arrbyBlockCipherText;
...
std::vector<uint8_t> Rijndael::Decrypt(const std::vector<uint8_t> &arrbyBlockCipherText, std::shared_ptr<std::vector<uint8_t>> byarrKey)
{
	// Create matrix State with size 4*Nb and Value in Cells is 0
	auto State = std::make_shared<std::vector<std::vector<uint8_t>>>(Nb, std::vector<uint8_t>(Nb, 0));
	...
	std::vector<uint8_t> arrBlockPlainText;
...
Rijndael::Rijndael(uint8_t valueNb, uint8_t valueNk, uint8_t valueNr)
{
	...
	Nk = valueNk;
	Nr = valueNr;
	RoundKeys = std::make_shared<std::vector<uint8_t>>((Nb * (Nr + 1) * Nb), 0); // create buffer of RoundKeys size Nb*(Nr+1)*Nb and Value in Cells is 0
};
