...
void SHA512::PaddingTheMessage()
{
	...
	while (byarrMessage->size() % 128 != 112)
	{
		byarrMessage->push_back(0x00);
	}
	...
	for (int i = 7; i > -1; i--)
	{
		byarrMessage->push_back(transformationNumbers.BytesOfNumber[i]);
	}
};
...
void SHA512::HashComplulationBlock()
{
	...
	{
		W->push_back(sigma1((*W)[t - 2]) + (*W)[t - 7] + sigma0((*W)[t - 15]) + (*W)[t - 16]);
	}
...
std::shared_ptr<std::vector<uint8_t>> SHA512::GetHash(std::shared_ptr<std::vector<uint8_t>> Message)
{

	byarrMessage = Message;
	M = std::make_unique<std::vector<uint64_t>>();
	W = std::make_unique<std::vector<uint64_t>>();
	...
	for (int i = 7; i > -1; i--)
	{
		Digest->push_back(formatDigest.byArray[i]);
	}
	formatDigest.qwH = H1;
	for (int i = 7; i > -1; i--)
	{
